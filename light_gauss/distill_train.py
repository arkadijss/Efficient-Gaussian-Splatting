...
from light_gauss.gaussian_renderer import render, network_gui
import sys
from light_gauss.scene import Scene, GaussianModel
...
from light_gauss.utils.graphics_utils import getWorld2View2
from light_gauss.utils.pose_utils import gaussian_poses
from icecream import ic 
import random
import copy
import json
import numpy as np
from light_gauss.utils.logger_utils import prepare_output_and_logger, training_report
from torch.optim.lr_scheduler import ExponentialLR
from light_gauss.prune import prune_list, calculate_v_imp_score
...
def training(args, dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, new_max_sh):
    ...
    with torch.no_grad():
        ...
        teacher_gaussians.load_ply(args.teacher_model)
        # teacher_gaussians.training_setup(opt)

    student_gaussians = GaussianModel(old_sh_degree)
    student_scene = Scene(dataset, student_gaussians, load_iteration=args.load_iteration)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--load_iteration", type=int,default=5000)
